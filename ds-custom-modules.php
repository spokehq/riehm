...
class DS_Custom_Module_Card extends ET_Builder_Module {
	function init() {
		...
		$this->main_css_element = '%%order_class%%.et_pb_card';
		...
		$this->custom_css_options = array(
			...
			'blurb_content' => array(
				'label'    => esc_html__( 'Location, Address, other Details', 'et_builder' ),
	...
	function get_fields() {
		...
		$fields = array(
			...
			'date' => array(
				'label'           => esc_html__( 'Dates', 'et_builder' ),
				'type'            => 'text',
				'option_category' => 'basic_option',
				'description'     => esc_html__( 'Ex: May 18 - Oct 12.', 'et_builder' ),
			),
			'time' => array(
				'label'           => esc_html__( 'Time', 'et_builder' ),
				'type'            => 'text',
				'option_category' => 'basic_option',
				'description'     => esc_html__( 'Ex: 4:00 - 7:00 PM', 'et_builder' ),
			),
			'frequency' => array(
				'label'           => esc_html__( 'Frequency', 'et_builder' ),
				'type'            => 'text',
				'option_category' => 'basic_option',
				'description'     => esc_html__( 'How often? Ex. "Every Wednesday"', 'et_builder' ),
			),
			'maplink' => array(
				'label'           => esc_html__( 'Map Link', 'et_builder' ),
				'type'            => 'text',
				'option_category' => 'basic_option',
				'description'     => esc_html__( 'Link to Google Map"', 'et_builder' ),
			),
			'use_icon' => array(
	...
	function shortcode_callback( $atts, $content = null, $function_name ) {
		...
		$frequency	           = $this->shortcode_atts['frequency'];
		$maplink	           = $this->shortcode_atts['maplink'];
		...
		if ( '' !== $frequency ) {
			$frequency = "<p class='article-card__category'>{$frequency}</p>";
		}
		if ( '' !== $maplink ) {
			$maplink = "<a href='{$maplink}' target='_blank'>See Map</a>";
		}
...
*/
		...
		$output = sprintf(
			...
			//'<div class="et_pb_blog_grid_wrapper"><div class="SARAH et_pb_blog_grid clearfix et_pb_card%9$s%11$s%12$s">
			'<div class="et_pb_blog_grid_wrapper divi-100-article-card">
			<div class="SARAH  clearfix et_pb_card">
				<article class="card-riehm et_pb_blog_grid et_pb_post clearfix post type-post location article-card">
					<div class="article-card__content">
						%6$s
						<h2 class="entry-title article-card__title">%2$s</h2>
						<div class="details">
							%4$s
							%5$s
						</div>
						<div class="article-card__excerpt">%1$s
							%7$s
						</div>
					</div>	
				</article>
...
*/
		...
		if ( '' !== $priceperweek ) {
			...
			$priceperweek = "<tr><td>Approx. price per week: </td><td><span class='strong'>{$priceperweek}</span></td></tr>";
		}
		if ( '' !== $run ) {
			//$run = "<dt>For: </dt><dd><span class='strong'>{$run}</span> <span class='extra-small'>weeks</span></dd>";
			$run = "<tr><td>For: </td><td><span class='strong'>{$run}</span> <span class='extra-small'>weeks</span></td></tr>";
		}
		
		if('' !== $accordiontitle) {
			//$accordiontitle = "<h3 class='heading-more'><span class='et_learnmore_arrow'><span></span></span>{$accordiontitle}<span class='icon'></span></h3>";
			$accordiontitle = "<h3 class='heading-more'>{$accordiontitle}<span class='icon'></span></h3>";
		}		
			...
			'<div%10$s class="csa-plan-description et_pb_blurb%9$s%11$s%12$s">
				<div class="et_pb_blurb_content">
					...
					<div class="et_pb_blurb_container">
						<table class="plan-description-list clearfix">
							%7$s
							%8$s
						</table>
						%5$s
...
class DS_Custom_Module_Upcoming_Events_Slider extends ET_Builder_Module {
	function init() {
		$this->name       = esc_html__( 'Events Slider', 'et_builder' );
...
class DS_Custom_Module_Upcoming_Events extends ET_Builder_Module {
	function init() {
		$this->name       = esc_html__( 'Events', 'et_builder' );
